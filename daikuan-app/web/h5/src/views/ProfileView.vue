...
const submittedApplications=computed(()=>store.applications.filter(item=>item.status!=='draft'))
const approvedAmount=computed(()=>submittedApplications.value.filter(item=>item.status==='approved').reduce((sum,item)=>sum+Number(item.amount),0))
const reviewingCount=computed(()=>submittedApplications.value.filter(item=>['manual','phone','funding'].includes(item.status)).length)
