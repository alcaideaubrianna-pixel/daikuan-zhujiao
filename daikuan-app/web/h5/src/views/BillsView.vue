...
const submitted=computed(()=>store.applications.filter(item=>item.status!=='draft'))
const current=computed(()=>active.value==='reviewing'?reviewing.value:active.value==='approved'?approved.value:active.value==='closed'?closed.value:submitted.value)
