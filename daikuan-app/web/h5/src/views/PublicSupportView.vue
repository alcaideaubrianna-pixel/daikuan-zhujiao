...
onMounted(async () => { await load(); if (token) { eventSource = new EventSource(`${(import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '')}/app/loan/support/public/stream?token=${encodeURIComponent(token)}`); eventSource.onmessage = event => { try { const item = map(JSON.parse(event.data)); if (!messages.value.some(existing => existing.id === item.id)) messages.value.push(item) } catch {} }; eventSource.onerror = () => { eventSource?.close(); eventSource = undefined; if (!refreshTimer) refreshTimer = setInterval(() => load(true), 3000) } } })
