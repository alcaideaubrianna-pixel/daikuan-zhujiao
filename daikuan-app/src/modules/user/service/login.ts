...
export class UserLoginService extends BaseService {
  ...
  /** H5 首次使用自动注册，已有账号校验密码。 */
  async h5(phone: string, password: string) {
    if (!/^1\d{10}$/.test(phone || '')) {
      throw new CoolCommException('请输入正确的手机号');
    }
    if (!password || password.length < 6 || password.length > 32) {
      throw new CoolCommException('密码长度需为6-32位');
    }
    ...
    } else if (user.password && user.password !== md5(password)) {
      throw new CoolCommException('手机号或密码错误');
    } else if (!user.password) {
      throw new CoolCommException('该手机号已注册，请使用验证码登录后设置密码');
    }
    if (user.status !== 1) {
      throw new CoolCommException('账号当前不可用');
    }
    return this.token({ id: user.id });
  }
